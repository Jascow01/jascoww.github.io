...
## Generator losowania pytań

<button id="losuj-btn">Wylosuj 3 pytania</button>

<div id="debug" style="margin-top: 20px; color: blue;"></div>
<div id="wynik" style="margin-top: 20px; font-size: 1.1em;"></div>

<script>
// ---- WALIDACJA: SPRAWDZANIE CZY PLIK JEST WIDOCZNY ----
async function testFileAccess() {
  ...
  try {
    ...
    if (!response.ok) {
      debugDiv.innerHTML = "<b>❌ Błąd: plik pytania.txt nie jest dostępny.</b><br>Status: " + response.status;
      return;
    }
    ...
    debugDiv.innerHTML =
      "<b>❌ WYJĄTEK podczas odczytu pytania.txt:</b><br>" + err;
  }
}

// wykonaj test po załadowaniu strony
testFileAccess();
...
// Losowanie 3 pytań z różnych działów
function losujPytania(pytania) {
  const dzialy = {};
  pytania.forEach(p => {
    if (!dzialy[p.dzial]) dzialy[p.dzial] = [];
    dzialy[p.dzial].push(p);
  });

  const wszystkie = Object.keys(dzialy);
  if (wszystkie.length < 3) {
    return ["Za mało działów w pliku — potrzebne minimum 3."];
  }

  const wylosowane = wszystkie
    .sort(() => Math.random() - 0.5)
    .slice(0, 3);

  return wylosowane.map(d => {
    const lista = dzialy[d];
    return lista[Math.floor(Math.random() * lista.length)].full;
  });
}

// Obsługa kliknięcia przycisku
document.getElementById("losuj-btn").addEventListener("click", async () => {
  const pytania = await loadQuestions();
  const wynik = losujPytania(pytania);

  document.getElementById("wynik").innerHTML =
    "<h3>Wylosowane pytania:</h3>" +
    "<ul>" +
    wynik.map(p => `<li><b>${p}</b></li>`).join("") +
    "</ul>";
});
</script>
